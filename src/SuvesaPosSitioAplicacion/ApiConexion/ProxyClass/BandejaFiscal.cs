...
public sealed class BandejaFiscal : ProxyBase, IBandejaFiscal
{
    ...
    public Task<ResponseGeneric<ResultadoBandejaFiscalDTO>> Consultar(FiltroBandejaFiscalDTO f) => Ejecutar(async () => await LeerJson<ResultadoBandejaFiscalDTO>(await _api.GetAsync($"api/comprobantes-electronicos/v44/bandeja?clave={Uri.EscapeDataString(f.Clave ?? string.Empty)}&estado={Uri.EscapeDataString(f.Estado ?? string.Empty)}&tipoComprobante={Uri.EscapeDataString(f.TipoComprobante ?? string.Empty)}&origenPosTipo={Uri.EscapeDataString(f.OrigenPosTipo ?? string.Empty)}{(f.OrigenPosId is { } id ? $"&origenPosId={id}" : string.Empty)}&pagina={f.Pagina}&tamanoPagina={f.TamanoPagina}")), "consultar la bandeja fiscal");

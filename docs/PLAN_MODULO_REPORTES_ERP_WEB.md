...
- `/moduloReportes` es hoy **26 tipos de reporte**, cada uno su propia página/ruta bajo `Views/Reportes/` (`/moduloReportes/<tipo>`, desde el 16 sep 2026 — antes eran pestañas de una sola página `Compras.razor` con `?tipo=` por query string), agrupados en 5 dominios visuales (Panel ejecutivo, Ventas y cartera, Caja y bancos, Compras, Inventario) dentro del componente compartido `ReporteOperacionPanel`, con filtros de fecha/sucursal/empresa/cliente/proveedor/artículo/bodega/lote/texto libre, indicadores (tarjetas de KPI), columnas de grilla propias por tipo y botones Exportar Excel/PDF — consume `GET api/reportes-operacion/{tipo}` vía `IReportesOperacion`/`ReportesOperacion.cs`.
...
1. **Exportar Excel/PDF da 404 en 9 de los ~19 tipos — ✅ resuelto (fecha no registrada, encontrado ya corregido el 16 sep 2026).** `Program.cs:401` amplió el whitelist a los 26 tipos que expone `ReportesOperacionController` (el propio comentario del código documenta el antes/después: "Antes solo cubría 11").
...
4. **Permisos planos, no por reporte — ✅ resuelto (14 sep 2026).** El nodo "Módulo Reportes" pasó de ítem único a grupo con hijos (uno por tipo, código `MODULO_REPORTES.<TIPO>`, generado con `tools/anotar_codigos_menu.py`). Cada hijo abre `/moduloReportes/<tipo>` — **actualización 16 sep 2026:** el hub dejó de leer `?tipo=` por query string en una sola página; cada tipo de reporte ahora es su propia página/ruta (`Views/Reportes/Ventas.razor`, `Compras.razor`, etc., 28 archivos), cada una instanciando el componente compartido `ReporteOperacionPanel` — ver el detalle de por qué en la sección de Experiencia de usuario más abajo. Los dominios visuales y el filtrado por permiso (`Sesion.PuedeVer("MODULO_REPORTES.<TIPO>")`) siguen viviendo en `ReporteOperacionPanel`; `comisiones` sigue con su propio permiso `VENTAS.COMISIONES`. El API exige el mismo código por endpoint (`ReportesOperacionController`, un `[ExigePermiso]` por acción).
...
**Estado (14 sep 2026): el árbol de permisos existe, está concedido a los 3 roles reales del sistema, y `Seguridad:ExigirPermisos` ya está activo.** El menú y el API distinguen los 25 tipos por código propio (`MODULO_REPORTES.<TIPO>`); los 3 roles que existen hoy (`Administrador`, `Admin_CostaPets`, `SA COSTA PETS`) tienen todo concedido — no solo Reportes, se auditaron los 10 módulos del catálogo completo antes de activar el flag, para no cortarle acceso a nadie por sorpresa. La matriz de abajo (Cajero, Contador, Jefe de facturación, etc.) sigue siendo la referencia para cuando el negocio cree roles más acotados que ese Administrador único — hoy no hay ningún rol al que discriminarle nada.
...
3. ✅ Construir el árbol `MODULO_REPORTES.*` (API) + convertir el nodo plano del menú en submenú filtrado por permiso (web) — hecho 14 sep 2026. ✅ Paso operativo también cerrado: los 25 códigos concedidos a los 3 roles existentes y `Seguridad:ExigirPermisos` activo (ver nota en "Usuarios, decisiones y permisos" arriba).

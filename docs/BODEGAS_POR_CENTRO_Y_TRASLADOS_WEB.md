...
|---|---|---|---|
| pendientes | _(este)_ | `LotesApiCliente.MantenimientoBodegasAsync(idSucursal?, incluirConsignacion)` (→ `api/mantenimientos/bodegas`). `Parametros/Bodegas.razor` reescrito: columna **Centro**, filtros "Ver todos los centros" / "Ver bodegas de consignación", badge Central/Cliente. Proxy `IMantenimientosInventario`/`MantenimientosInventario` retirado (+ DI). `Ajuste.razor`: `<details>` "Reponer la bodega de consignación central" — `AbrirBodegaCentralAsync` al abrir (muestra existencia), form origen + artículos + `ReponerCentralAsync`. DTOs `BodegaCentralConsignacion` / `AbrirBodegaCentralConsignacion` / `ReponerCentralConsignacionRequest(+Linea)` + métodos en `IConsignacionInvApiCliente`. | `dotnet build` 0 err · `dotnet test` 72/72 |

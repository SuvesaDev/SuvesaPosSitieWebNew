...
|---|---|---|---|
| pendientes | `02f8b2d` + `c15e3c3` | `Parametros/Bodegas.razor` reescrito: columna **Centro**, toggle "Ver todos los centros"; usa `BodegasOperativasAsync` (`bodega/ObtenerBodegas`, POST, JWT — ya trae `idSucursal`/`nombreSucursal`). Proxy `IMantenimientosInventario`/`MantenimientosInventario` retirado (+ DI). `Ajuste.razor`: `<details>` "Reponer la bodega de consignación central" — `AbrirBodegaCentralAsync` al abrir (muestra existencia), form origen + artículos + `ReponerCentralAsync`. DTOs `BodegaCentralConsignacion` / `AbrirBodegaCentralConsignacion` / `ReponerCentralConsignacionRequest(+Linea)` + métodos en `IConsignacionInvApiCliente`. **No se usa `api/mantenimientos/bodegas`** (devolvía 401 por el esquema de auth; corregido en el API `205581f3`, pero la pantalla no lo necesita y las bodegas de consignación no van aquí — req #6). | `dotnet build` 0 err · `dotnet test` 72/72 |
| §3.2 | `f80a550` | `Facturacion.razor`: `<select>` **Bodega** en la cabecera (oculto en modo consignación), cargado con `Lotes2.BodegasOperativasAsync(Sesion.IdSucursal)`, default = primera; guard en `Emitir` (no consignación ⇒ bodega obligatoria); cada `FacturaDetallesDTO.IdBodega = _bodega` (venta y preventa/prefactura). `Compra.razor`: ya tenía selector de bodega — se cambió la fuente de `Api.Bodegas(costaPets)` a `Lotes2.BodegasOperativasAsync(Sesion.IdSucursal)` (centro-filtrado, sin consignación); `FacturaCompraDetalleDTO.IdBodega = _bodega` ya estaba. `Proforma.razor`: **N/A** — la cotización es un presupuesto puro (`DetalleCotizacionDTO` sin bodega, no toca inventario); la reserva de preventa con bodega la cubre `Facturacion` (`Preventa=true`). API: `BodegaPorDefecto()` de ventas excluye consignación (DevSuvesaPosWeb `dc9adaaf`). | `dotnet build` 0 err · `dotnet test` 72/72 |
| §3.5 (centro) | `7d6cfbf` | `Consignacion/Bodegas.razor`: `AbrirBodegaAsync` manda `IdSucursal = Sesion.IdSucursal` para que la bodega de consignación del cliente quede ligada a su centro (DTO `AbrirBodegaConsignacion` gana `IdSucursal`). El API la liga también en el camino idempotente (repara las ya creadas). | `dotnet build` 0 err · `dotnet test` 72/72 |

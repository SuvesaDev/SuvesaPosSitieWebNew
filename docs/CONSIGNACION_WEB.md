...
| §6a (facturar) | `<pend>` | `IConsignacionInvApiCliente`: métodos de prefactura (`GenerarPrefactura` / `EditarPrefactura` / `AprobarPrefactura` / `FacturarPrefactura` / `AnularPrefactura` / `Prefactura`) + DTOs. **`Prefactura.razor`** (`/consignment/prebill`, código `CONSIGNACION.FACTURACION_DE_CONSIGNACIONES` — el ítem de menú se repunta a esta ruta): abre una prefactura por N.º, muestra líneas y totales con badge de estado, y según el estado ofrece **Aprobar** (1→2), **Facturar** con condición Contado/Crédito + plazo (2→3) y **Anular** (modal con motivo). | build + 72 tests |
| §6b (generar) | `<pend>` | `Views/Ventas/Facturacion.razor` gana la ruta `@page "/consignment/prebill/from/{ConteoId:long}"` y un modo **prefactura de consignación** (`EsPrefacturaConsignacion => ConteoId > 0`; `EsConsignacion` lo incluye). `OnParametersSetAsync` → `CargarConteoPrefactura`: trae el conteo (`ConteoAsync`), resuelve el cliente por `IdCliente`, y precarga una línea por cada artículo con `Vendido > 0` (`AgregarArticuloConLotes` si la línea trae lote; precio = tarifa del cliente, o `PrecioUnitario` del conteo si viene). El cajero ajusta precios y agrega bonificaciones con los modales existentes (mismo motor). En prefactura mode se saltan las validaciones de crédito/morosidad/orden de compra (la condición va al facturar) y `Preventa = true`. "Emitir" → botón "Generar prefactura" → `ConsignacionApi.GenerarPrefacturaAsync({ IdConteo, Factura })` → navega a `/consignment/prebill?id=<nueva>`. `InventarioFisico.razor` gana el botón "Generar prefactura" (conteo en Borrador con vendido > 0) que abre esa ruta. | build + 72 tests |
| §7 (usabilidad) | `eec84e4` | API: endpoint `Prefacturas` (POST, `ListarPrefacturas`) — commit API `4e28c7c4`. Web: `IConsignacionInvApiCliente.PrefacturasAsync` + `Prefactura.razor` gana una **tabla de prefacturas** (filtro por estado; por defecto Editable + Aprobada), abrir por fila o por N.º, se refresca tras aprobar/facturar/anular. `Ajuste.razor` gana **"Anular boleta por N.º"** (abrir + anular con modal de motivo; muestra estado ANULADA). | build + 72 tests |
| §8 | `3b3f6de` | **Retiro del flujo viejo.** Borrados `Views/Consignacion/Seguimiento.razor`, proxy `IConsignaciones` + `Consignaciones`, registros DI (`IConsignaciones`, `IConsignacionApiCliente` NSwag). Menú: se quitan `CONSIGNACION.REGISTRO_DE_CONSIGNACIONES` y `.SEGUIMIENTO_DE_CONSIGNACIONES`; `CONSIGNACION` queda con Bodegas / Ajuste / Inventario Físico / Kardex / Facturación. `FiltroMenuTests` 13 raíz / **107** nodos. `Facturacion.razor`: se quita la ruta `/buys/consignment/register`; `EsConsignacion` es ahora sólo el modo prefactura de consignación. API en paralelo: commit `25a657ab` retira `ConsignacionController` + `ConsignacionManager`. | build + 72 tests |

### Notas (post-§8)

- La clase NSwag generada `ConsignacionApiCliente` sigue en `Generated/*` — compila
  sola y sale en la próxima regeneración. Los códigos de menú
  `REGISTRO_DE_CONSIGNACIONES` / `SEGUIMIENTO_DE_CONSIGNACIONES` siguen en el seed
  (inofensivo; `MenuCodigosTests` sólo verifica menú → seed).
- El API dejó vivos (dead-ish) `venta/*Consignacion*` + `ConsignacionEncabezadoDTO`
  y afines; ya nadie los llama desde el sitio.

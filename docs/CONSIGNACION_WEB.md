...
| §6b (generar) | — | **Pendiente**: generar la prefactura desde un conteo. Necesita armar el `FacturaDTO` con el contexto de facturación (empresa/sucursal/caja/moneda/tipo), el cálculo de impuestos por línea y la **bonificación expandida** — todo eso ya vive en `Views/Ventas/Facturacion.razor`. La vía correcta es **extender `Facturacion.razor`** con un modo "prefactura de consignación" (ruta `/consignment/prebill/from/{conteo}`) que precargue cliente + líneas del conteo (`Vendido`) y, en "Emitir", llame `GenerarPrefacturaAsync` en vez del emit normal. Es un cambio delicado sobre una pantalla crítica → tarea enfocada aparte. |

...
| §8 | — | **Pendiente**: retirar `Seguimiento.razor` + proxy `IConsignaciones` y los ítems de menú `CONSIGNACION.REGISTRO_DE_CONSIGNACIONES` (→ `Facturacion.razor` modo consignación viejo) / `CONSIGNACION.SEGUIMIENTO_DE_CONSIGNACIONES`, cuando el API retire sus endpoints legacy (`ConsignacionController.*`). Hoy conviven con el flujo nuevo. |

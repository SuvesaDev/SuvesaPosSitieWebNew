...
| Consignación | `ContratosOperacionDiariaTests.ConsignacionDeCarne_DesdeLaCentralHastaFacturaCredito_ConservaLoteCantidadYEstados` simula central → bodega del cliente → ingreso → conteo → prefactura → aprobación → factura a crédito y kardex. | `ImpresionRenderTests` cubre la boleta de consignación en ambos formatos. |

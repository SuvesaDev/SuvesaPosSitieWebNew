...
Fecha: 2026-09-05. Estado: W0 cerrado; API A1–A7 + soporte W5 code-complete; **W1–W3 y W5 completas; W4-W7 completas salvo E2E sandbox**.
...
| W6 | ✅ | Pantalla `EstadoCuenta.razor` (`/sales/account-statement[/{id}]`, gated `VENTAS.ABONO_COBRAR`): busca cliente, fecha de corte, tiles de límite/saldo/crédito a favor/disponible, tramos de antigüedad y detalle de facturas con saldo. Fuente única `api/cobros/estado-cuenta/{id}`. **Exportación:** "Descargar PDF" (`/reportes/estado-cuenta?idCliente=&corte=` — endpoint minimal-API con `IGeneradorPdf.Tabla`, mismo corte) gateado por `Sesion.Puede("VENTAS.ABONO_COBRAR", Imprimir)` y "Descargar CSV" gateado por `Exportar`. Enlace desde `CuentasPorCobrar`. |
| W4 | ✅ | `PreparacionPagoVenta` (servicio puro §4): `Calcular(total, líneas)` → reparto con recibido/aplicado/vuelto/faltante por forma; invariante `recibido − vuelto = aplicado`; vuelto solo de efectivo; sobrepago no-efectivo y referencia faltante son error. 8 casos en `PreparacionPagoVentaTests` (E02 incluido). Componente Blazor **`PanelPagoVenta.razor`** (montos + referencia por forma no-efectivo + pagado/vuelto/falta + errores), con `@bind-Montos`/`@bind-Referencias` y reparto vía el servicio. Cableado en el bloque de tiquete de `Facturacion.razor` y en la captura del lote de preventas de `CuentasPorCobrar`; la referencia viaja en `PagoFacturaDTO.Referencia` / `PagoPreventaContadoDTO.Referencia`. |

| W5 | ✅ | API: `ServicioFacturarPreventaContado` + `POST api/venta-orquestada/facturar-preventa-contado` — una llamada idempotente que cobra (Cobro/FormasPago/Aplicación + MovimientoCaja), marca `Cobrado` y factura la preventa; rechaza crédito / anulada / sin apertura / pago < 100%; pulsa la señal fiscal. 11 tests. Web: contratos + `IComandosFacturacion.FacturarPreventaContado`; **ambas** pantallas convergen a este comando: `CuentasPorCobrar.CobrarYFacturar` (una llamada por preventa, clave de idempotencia por lote) y `Cobrar.Confirmar` (rama de contado; la de crédito sigue siendo abono con `Api.Cobrar`). Ya no se encadena `Cobrar → FacturarPreventa → Emitir` en la vista. Pendiente menor: separar los permisos `CAJA.COBRAR` / `VENTAS.ABONO_COBRAR` de las dos rutas. |
| W7 | ✅ | La distinción interno/electrónico la resuelve el API (A3) y la web la refleja: `MostrarResultadoOperacion` en Facturación y aviso en Cobrar ("Documento interno, sin espera de Hacienda" vs "Comprobante electrónico en proceso"); nunca XML ni "pendiente" para `NoAplica`. **PDF recuperable + visor embebido:** componente `VisorPdf.razor` abre `/documentos/{slug}/{id}/pdf` en un modal con `<iframe>` (más "Abrir en pestaña"), usado en el resultado de `Cobrar` y en la tabla de `CuentasPorCobrar`; idempotente, no re-cobra. Correo de crédito al aceptar: confirmado en el API (`AceptadosAsync` filtra por estado, no por cobro). Falta: estados de correo pendiente/enviado/fallido separados en la bandeja. |

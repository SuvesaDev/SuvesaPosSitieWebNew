...
Fecha: 2026-09-05. Estado: W0 cerrado; API A1–A7 + soporte W5 code-complete; **W1–W3 y W5 completas; W4/W6 casi; W7 núcleo hecho**.
...
| W6 | 🟡 casi | Pantalla `EstadoCuenta.razor` (`/sales/account-statement[/{id}]`, gated `VENTAS.ABONO_COBRAR`): busca cliente, fecha de corte, tiles de límite/saldo/crédito a favor/disponible, tramos de antigüedad y detalle de facturas con saldo. Fuente única `api/cobros/estado-cuenta/{id}`. **Exportación:** "Descargar PDF" (`/reportes/estado-cuenta?idCliente=&corte=` — endpoint minimal-API con `IGeneradorPdf.Tabla`, mismo corte) y "Descargar CSV" (cliente, `seepos.descargarTexto`). Enlace desde `CuentasPorCobrar`. Falta: permisos VER/EXPORTAR/IMPRIMIR separados, y confirmar si hay reporte legado que reutilizar. |
| W4 | 🟡 casi | `PreparacionPagoVenta` (servicio puro §4): `Calcular(total, líneas)` → reparto con recibido/aplicado/vuelto/faltante por forma; invariante `recibido − vuelto = aplicado`; vuelto solo de efectivo; sobrepago no-efectivo y referencia faltante son error. 8 casos en `PreparacionPagoVentaTests` (E02 incluido). Componente Blazor **`PanelPagoVenta.razor`** (montos + referencia por forma no-efectivo + pagado/vuelto/falta + errores), cableado en el bloque de tiquete de `Facturacion.razor` con `@bind-Montos`/`@bind-Referencias` y reparto vía el servicio; la referencia viaja en `PagoFacturaDTO.Referencia`. Falta: usarlo también en `CuentasPorCobrar`/`Cobrar` (hoy reparto en cascada multi-documento). |

| W5 | ✅ | API: `ServicioFacturarPreventaContado` + `POST api/venta-orquestada/facturar-preventa-contado` — una llamada idempotente que cobra (Cobro/FormasPago/Aplicación + MovimientoCaja), marca `Cobrado` y factura la preventa; rechaza crédito / anulada / sin apertura / pago < 100%; pulsa la señal fiscal. 11 tests. Web: contratos + `IComandosFacturacion.FacturarPreventaContado`; **ambas** pantallas convergen a este comando: `CuentasPorCobrar.CobrarYFacturar` (una llamada por preventa, clave de idempotencia por lote) y `Cobrar.Confirmar` (rama de contado; la de crédito sigue siendo abono con `Api.Cobrar`). Ya no se encadena `Cobrar → FacturarPreventa → Emitir` en la vista. Pendiente menor: unificar la captura de pago con `PanelPagoVenta` y separar permisos `CAJA.COBRAR` / `VENTAS.ABONO_COBRAR`. |
| W7 | 🟡 núcleo hecho | La distinción interno/electrónico la resuelve el API (A3) y la web la refleja: `MostrarResultadoOperacion` en Facturación y aviso en Cobrar ("Documento interno, sin espera de Hacienda" vs "Comprobante electrónico en proceso"); nunca XML ni "pendiente" para `NoAplica`. **PDF recuperable:** `CuentasPorCobrar` ya tenía `<BotonImprimir>` persistente por fila; `Cobrar` ahora deja una tarjeta de resultado con "Ver comprobante (PDF)" que apunta a `/documentos/{slug}/{id}/pdf` (idempotente, no re-cobra). Correo de crédito al aceptar: confirmado en el API (`AceptadosAsync` filtra por estado, no por cobro). Falta: visor PDF embebido (hoy pestaña nueva) y estados de correo separados en la bandeja. |
